...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	public DisposableBeanAdapter(Object bean, String beanName, RootBeanDefinition beanDefinition,
		...
		this.invokeDisposableBean =
				//实现了 DisposableBean && 没有叫 destroy 的被 @PreDestroy 注解的方法
		...
		if (destroyMethodName != null && !(this.invokeDisposableBean && "destroy".equals(destroyMethodName)) &&
			...
			if (destroyMethod == null) {
				// 如果destroy方法名为空，并且enforceDestroyMethod(强制调用销毁方法)为true，则抛出异常
	...
	private String inferDestroyMethodIfNecessary(Object bean, RootBeanDefinition beanDefinition) {
		...
		if (AbstractBeanDefinition.INFER_METHOD.equals(destroyMethodName) ||
			...
			// 如果bean没有实现DisposableBean接口，则尝试推测destroy方法的名字
			if (!(bean instanceof DisposableBean)) {
				try {
					// 尝试在bean中寻找方法名为close的方法作为destroy方法
					return bean.getClass().getMethod(CLOSE_METHOD_NAME).getName();
				}
				catch (NoSuchMethodException ex) {
					try {
						// 尝试在bean中寻找方法名为close的方法作为shutdown方法
	...
	private List<DestructionAwareBeanPostProcessor> filterPostProcessors(List<BeanPostProcessor> processors, Object bean) {
		...
		if (!CollectionUtils.isEmpty(processors)) {
			...
			for (BeanPostProcessor processor : processors) {
				// 如果processor是DestructionAwareBeanPostProcessor
				if (processor instanceof DestructionAwareBeanPostProcessor) {
					DestructionAwareBeanPostProcessor dabpp = (DestructionAwareBeanPostProcessor) processor;
					if (dabpp.requiresDestruction(bean)) {// InitDestroyAnnotationBeanPostProcessor
	...
	public static boolean hasDestroyMethod(Object bean, RootBeanDefinition beanDefinition) {
		if (bean instanceof DisposableBean || bean instanceof AutoCloseable) {
			// 如果bean实现了DisposableBean接口 或者 bean是AutoCloseable实例，则返回true，因为这两个接口都有关闭的方法
			return true;
		}
